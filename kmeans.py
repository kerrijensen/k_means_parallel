...
def calculateDistance(x_coords, y_coords, x_centroids, y_centroids):
    with multiprocessing.Pool() as pool:
        tasks = [(i, x_coords, y_coords, x_centroids[i], y_centroids[i]) for i in range(len(x_centroids))]
        results = pool.map(calculate_centroid_distances, tasks)

    distances = list(map(list, zip(*results)))
    return distances


def assignNodes(x_coords, y_coords, x_centroids, y_centroids, distances):
    assignments = []
